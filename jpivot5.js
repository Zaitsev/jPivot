...
08.05.2010 17:43:04	 some	optimaizings for //create	mapping	data to	pvtable	routine
...
///	aggregators	setup
  ...
  function jpv_pivotDrawData()
  {
    ...
    function append_total_row	(col,key_ind)
    {
      ...
      table_data[add][col]=['&Sigma;&nbsp;'+pv.rows_totals[col][key_ind][col],'colspan="'+ (row_keys_length-col) +'" keyind="'+col+'"	totind="'+key_ind+'" '];
      ...
      for	(var r=0;	r	<	td_rows_count; r++)
        {
          ...
          if (r	<	td_data_rows_start )
            {
              if (r	== tc)
                table_data[rn][add]=['&Sigma;&nbsp;'+pv.cols_totals[r][key_ind][r],'rowspan="'+	(col_keys_length-tc) +'" keyind="'+r+'"	totind="'+key_ind+'" '];
              else
        ...
        for	(var c=0;	c	<	td_cols_count	 ; c++)
          {
            ...
            tr[col_index]	=	'<td id="jpivot_rc_'+r+'_'+c+'"	c="'+c+'" r="'+r+'"'+param+'>'+val+'</td>';
            col_index++;
          }
