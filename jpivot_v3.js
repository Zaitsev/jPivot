...
16.04.2010 2:07:56 done - create average aggregate function for AVG when counts in data_col_cnt (pass it as aggregateTotalParam:{type:'AVG',precision:2,cnt_col:data_col_cnt}
...
16.04.2010 2:08:18 - if (typeof aggregate(Val|Totals)Param.params.precision	=== 'undefined')  move to prepare runtime
*/
...
// Private Variables and Functions
    ...
    function jpv_preparePv()
        ...
        if ($this.opts.onBeforePrepare)
            $this.opts.onBeforePrepare();
        //////////////// datt processing /////////////////
        //sort
        ...
        */
        ...
        composite_row_key_uniq_len=0;
        var composite_col_key_uniq=[];
        composite_col_key_uniq_len=0;
        ...
        for (dr=0; dr < data_length ; dr++)
            ...
            if ( ($this.opts.onCustomFilter) && ($this.opts.onCustomFilter (dr) ) )
                continue;
        ...
        if  ( (typeof($this.opts.aggregateValParam.precision)	=== 'undefined') || ($this.opts.aggregateValParam.precision==null))
                    $this.opts.aggregateValParam.precision=2;  
        $this.opts.aggregateValParam.precision_k = Math.pow(10,$this.opts.aggregateValParam.precision) 
        if  ( (typeof($this.opts.aggregateTotalParam.precision)	=== 'undefined') || ($this.opts.aggregateTotalParam.precision==null))
                    $this.opts.aggregateTotalParam.precision=2;  
 				$this.opts.aggregateTotalParam.precision_k = Math.pow(10,$this.opts.aggregateTotalParam.precision)                                                          
        //$this.opts.pivot_data = pv;
        if ($this.opts.precalculateTotals)
            jpv_precalculateTotals ($this.opts);
        if ($this.opts.onAfterPrepare)
            $this.opts.onAfterPrepare();
        }
    ...
    function jpv_pivotDrawData()
        ...
        var total_index=[];
        for (c=row_keys_length-1; c >= 0; c--)
            total_index[c]=0;
        var total_mask=[];
        for (r=0; r<row_keys_length; r++)
            total_mask[r]=opts.totals_mask[opts.rows[r]];
        var td_rows_map=[];
        for (r=0; r<td_data_rows_start+1; r++)
        ...
        for (c=row_keys_length-1-1; c >= 0; c--)
            ...
            is_tot=0;
            span=1;
            r_index=td_rows_map[td_data_rows_start][0]; //point to the first row with data - it always have keys.
            for (r=td_data_rows_start+1; r<td_rows_count; r++)
                {
        ...
        var key_i_group=[];
        for (c=0; c < col_keys_length-1-1; c++)
        ...
        if (opts.ExternalManage)
            ...
            for (i=0; i < col_keys_length; i++)
                table_data[i][1] = opts.onPrintKeyHeader (c,opts.data_headers[opts.cols[c]]);
            for (i=0; i < row_keys_length; i++)
                table_data[ck][i] = opts.onPrintKeyHeader (c,opts.data_headers[opts.rows[c]])
        ...
        if ($this.opts.onAfterDraw)
            $this.opts.onAfterDraw();
    ...
    $.fn.jPivot= function ( pin )
        ...
        return this.each
            ...
            var opts = $.extend (true,
                 ...
                 *
                * @param  data_indexes *
                * @return [value,html_ads_for_td]
                */
                ,onPrintValue:
                function (data_indexes)
                    {
                    return jpv_print_val.apply (this,arguments);
                    }
                //support functions for reuse in on-events

                /**
                 * return raw aggregated value for reusing in custom onPrintValue
                * @param  data_indexes
                * @return float or null if no value
                * example usage in custom onPrintValue event
                       ,onPrintValue:function(data_indexes)
                            {
                            var val = this.jpivot_PrintValue(data_indexes);
                            if (val == null )return ['','style="color:red;"'];
                            return [val ,'style="color:red;"'];
                            }
                */
                ,jpivot_PrintValue:function (data_indexes)
                    {
...
//public methods
    ...
    function jpv_aggregatePERC_val (cell_val,cell_obj)
        ...
        var ret =  tot ?	 Math.round ( $this.opts.aggregateValParam.precision_k * cell_val/tot ) / $this.opts.aggregateValParam.precision_k  : null;
        if (debug==3)
    ...
    function jpv_aggregateSUM_val (data_indexes)
		...
		function jpv_aggregateAVG_val (data_indexes)
		{
			...
			if( (typeof(this.aggregateValParam.data_col_cnt) !== 'undefined') &&  (this.aggregateValParam.data_col_cnt != null))
				{
			  //with precalc avg method
				for (var i=0; i < len; i++)
					{
						ret += parseFloat (this.data[data_indexes.cell_value[i]][data_col]);
						cnt += parseFloat (this.data[data_indexes.cell_value[i]][this.aggregateValParam.data_col_cnt ]);
					}
				}
			else
				{
        ...
        var ret_t = cnt ? Math.round (  $this.opts.aggregateValParam.precision_k  * ret/cnt)  / $this.opts.aggregateValParam.precision_k  : 0;
        if (debug==3)
    ...
    function jpv_aggregate_val	(data_indexes,aggregateValParam)
        ...
        switch (aggregateValParam.type)
            ...
            case 'SUM':
                ...
                break;
            case 'AVG':
                ret=jpv_aggregateAVG_val.call (this,data_indexes);
                break;
            case 'CUSTOM':
...
//extend opts with default events and props
    ...
    function jpv_precalc_totals_cells (opts,arr)
        ...
        var len=arr.length;
        for (var i=0 ; i < len ; i++)
            arr[i] = jpv_aggregate_total.call (opts,arr[i]);
        }
    ...
    function jpv_aggregateAVG_total (data_indexes_tot)
        ...
        var cnt=0;
        //simple or with precalc avg method
        var data_col_cnt= (typeof(this.aggregateTotalParam.data_col_cnt) !== 'undefined') ? this.aggregateTotalParam.data_col_cnt : null;
        for (i=0; i < len; i++)
            {
            vals = data_indexes_tot[i];
            if (typeof vals === 'undefined')
        ...
        var ret_r = cnt ? Math.round (  $this.opts.aggregateTotalParam.precision_k  * ret/cnt ) / $this.opts.aggregateTotalParam.precision_k  : 0;
        if (debug==3)
    ...
    */
    ...
    function jpv_print_total (data_indexes,rclass,dbg_prefix)
        ...
        var ret = jpv_aggregate_total.call (this,data_indexes);
        if (ret==null)
            ret='';
        return  [ret,rclass];
        }
