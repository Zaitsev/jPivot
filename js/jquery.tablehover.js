		...
		return this.each(function() 
        {
			...
			var addToIndex = function(rows, nodeName)
			{
				...
				for ( rowI = 0; rowI < rows.length; rowI++, rCnt++ )
				{
					...
					for ( cI = 0; cI < row.cells.length; cI++ )
					{
						...
						{
							//s = c.colSpan;
							s = parseInt($(c).attr("cspan")) || c.colSpan;
							var nexcl = ! ($(c).is('.pv_Key_cs') );
							while ( nexcl  && --s >= 0 )
							{
								rI = c.realIndex + s;
			...
			var highlight = function(cell, on)
			{
				...
				if ( settings.colClass != '' )
				{
					...
					var cspan = parseInt($(cell).attr("cspan")) || cell.colSpan;
					while (settings.spanCols && ++i < cspan && colIndex[cell.realIndex + i] )
					{
