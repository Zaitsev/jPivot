                ...
                $('#pv_dlg_plh'+k).dialog
                     ...
                     ,buttons: {
                                Ok: function()
                                    ...
                                    if ($this.opts.immediate_draw) {jpv_preparePv($this);$this.opts.nDrawData($this);}
                                    $(this).dialog('close');
                                    }
                                ,Cancel: function()
                                    {
...
function jpv_save($this)
     ...
     t.aggregate = $this.opts.aggregate;
...
function jpv_preparePv()
       ...
       for (var i=0, len= pre_uniquie_keys.length; i < len; i++)
            	  ...
            	  for(var j  in pre_uniquie_keys[i]) 
            	  		{
            	  		pv.unique_keys[i].push(pre_uniquie_keys[j]);
            	  		}
...
*/   
 ...
 /* TODO some overworking in cols_keys fill */
        ...
        function fill_cnt (rw)
               {
               if (typeof pv.data_rows_totals_pos[rw]==='undefined')pv.data_rows_totals_pos[rw]=[];
               for(var i = row_keys_length; i < pv.data_row_length; i++) 
               				 {
               				 pv.rows_totals[c][cur_key][i].push(pv.data[rw][i])
               				 if (typeof pv.data_rows_totals_pos[rw][i]==='undefined')pv.data_rows_totals_pos[rw][i]=[];
         ...
         function fill_cnt_col (col)
               ...
               for(var i=col_keys_length; i < pv.data_rows_count; i++)
                   ...
                   pv.cols_totals[r][cur_key][i].push(pv.data[i][col]);
		               if (typeof pv.data_cols_totals_pos[i]==='undefined')pv.data_cols_totals_pos[i]=[];
		               if (typeof pv.data_cols_totals_pos[i][col]==='undefined')pv.data_cols_totals_pos[i][col]=[];
	...
	$.fn.jPivot= function( pin )
            ...
            /*								
					  ...
					  */
						...
						,getDialogFilter:function(data_row_length)//ac tas EXCLUDE filter with multiple values
								{
...
$.jpivot.extend
			...
			,setGridParam : function (newParams)
					...
					return this.each
						(function(){
								if (this.opts && typeof(newParams) === 'object') {$.extend(true,this.opts,newParams);}
						});
					}
			,drawData : function()
			    {
